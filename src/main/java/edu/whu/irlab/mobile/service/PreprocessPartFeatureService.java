 ...
 */
public class PreprocessPartFeatureService extends AbstractPreprocessService {
    private static Logger logger = LoggerFactory.getLogger(PreprocessPartFeatureService.class);

    private static PreprocessPartFeatureService instance = new PreprocessPartFeatureService();

    private PreprocessPartFeatureService(){}

    public static PreprocessPartFeatureService getInstance(){
        return instance;
    }

    /**
     * 43个特征: 依赖前3个月数据, 从中选出部分特征
     * @param bigDataFiles
     * @return
     * @throws IOException
     */
    ...
    String getFeatureRecordInit(String mobile, List<String> lines) {
        ...
        for (String line: lines){
            ...
            if (props[3].equals("动感地带品牌")){
                singleLineFeature.add("1,0,0");
            }else if (props[3].equals("全球通品牌")){
                singleLineFeature.add("0,1,0");
            }else if (props[3].equals("神州行品牌")){
                singleLineFeature.add("0,0,1");
            }else{
                singleLineFeature.add("0,0,0");
            }
